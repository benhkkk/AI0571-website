// Pages Function: GET /api/send-test?to=邮箱 -> 立即发一封日报邮件（调试用）
// 绑定：Pages 项目 Settings -> Functions -> KV "SUBS" + Secret "RESEND_API_KEY"
...
export async function onRequestGet({ request, env }) {
  ...
  if (!env.RESEND_API_KEY) return json({ ok: false, error: 'missing RESEND_API_KEY (在 Pages 项目 Settings 配置)' });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to)) return json({ ok: false, error: '请提供 ?to=有效邮箱' });
  ...
  try {
    ...
    return json({ ok: false, error: 'fetch data.json failed: ' + String(e) });
  }
